...
import { useDashboardMetrics, useUsers, useBusinessApprovals, useSupportTickets } from '@/services/admin.service';
  ...
  const quickActions = [
    {
      ...
      count: businessApprovals?.filter(a => a.status === 'pending').length || 0
    },
    {
      id: 'resolve-disputes',
          ...
          <TabsContent value="pending-approvals" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">Business applications awaiting review</p>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {businessApprovals?.filter(a => a.status === 'pending').map((approval) => (
                    <div key={approval.id} className="flex items-center justify-between p-4 border rounded-lg">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <div className="font-medium">{approval.businessName}</div>
                          <div className="text-sm text-muted-foreground">
                            Owner: {approval.ownerName} • {approval.submittedAt}
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center space-x-2">
